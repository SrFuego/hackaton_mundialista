...
class UserBet(TimeStampedModel):
    ...
    match = models.ForeignKey(
        Match, null=True, blank=True, db_index=True, on_delete=models.SET_NULL)
    country = models.ForeignKey(
        Country, null=True, blank=True, db_index=True,
        on_delete=models.SET_NULL)
    is_active = models.BooleanField("Esta activo", default=True)
    privacity_status = models.CharField(
        choices=PRIVACITY_CHOICES, max_length=20)
    score_status = models.CharField(choices=SCORE_CHOICES, max_length=20)
    bet_status = models.CharField(choices=BET_CHOICES, max_length=20)
